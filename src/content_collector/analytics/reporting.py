...
class ReportGenerator:
    ...
    async def generate_tree_report(self, run_id: str) -> Dict[str, Any]:
        ...
        try:
            async with db_manager.session() as session:
                ...
                # Create a mapping of URL to page for easy lookup
                ...
                # If no parent_id relationships found, build relationships based on
                # depth and URL patterns
                if not parent_child_found:
                    ...
                    for depth in sorted(depth_groups.keys())[1:]:  # Skip depth 0
                        for page in depth_groups[depth]:
                            ...
                            # If no referer relationship found, try to find best parent
                            # from previous depth
                            if (
                                not parent_found
                                and depth > 0
                                ...
                                for potential_parent in depth_groups[depth - 1]:
                                    if potential_parent.domain == page.domain:
                                        ...
                                        # Check if page URL starts with parent URL
                                        # (path containment)
                                        if (
    ...
    def generate_tree_text(
        ...
        lines.append("=" * 80)
        lines.append("CONTENT COLLECTOR - TREE REPORT")
        lines.append("=" * 80)
